...
public class TileShader extends Shader {
	...
	public TileShader() throws Exception {
		...
		if(programId == 0) {
			throw new Exception("Could not create TileShader");
		}
		
		
		createShaderCode(FileUtils.readStream(Shader.class.getResourceAsStream("/shaders/tile/tile_fragment.fs")), GL20.GL_FRAGMENT_SHADER);
		createShaderCode(FileUtils.readStream(Shader.class.getResourceAsStream("/shaders/tile/tile_vertex.vs")), GL20.GL_VERTEX_SHADER);
